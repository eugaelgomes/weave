...
const { getInternalToolDefinitions, executeInternalTool } = require("../../../core/tools/tool-dispatcher");
...
async function researcherNode(state) {
  ...
  const readOnlyTools = availableTools.filter(t => !t.name.includes("create") && !t.name.includes("update") && !t.name.includes("delete"));
    ...
    if (data.type === "function_call" && data.toolCalls) {
      for (const tc of data.toolCalls) {
        try {
          ...
          const result = await executeInternalTool(tc.name, tc.arguments, executionContext);
          collectedData.push({
            source: tc.name,
            query: tc.arguments,
            result
          ...
          logger.warn("Researcher tool call failed", { name: tc.name, error: e.message });
