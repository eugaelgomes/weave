...
async function orchestratorNode(state) {
  ...
  try {
    const { data, provider } = await callAIProvider({
      ...
      prompt,
      systemMessage: "You are a JSON-only decision engine. Always return valid JSON.",
    ...
    logger.info("Orchestrator decision", { nextNode: decision.nextNode, reasoning: decision.reasoning });
