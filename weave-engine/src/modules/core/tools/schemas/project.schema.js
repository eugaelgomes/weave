...
const getProjectDetailsZodSchema = z.object({
  projectId: z.string().describe("The UUID of the project to retrieve details for."),
});
...
const createProjectZodSchema = z.object({
  ...
  status: z.enum(["OPEN", "IN_PROGRESS", "PAUSED", "COMPLETED", "ARCHIVED"]).optional().describe("The initial status of the project (must be one of: 'OPEN', 'IN_PROGRESS', 'PAUSED', 'COMPLETED', 'ARCHIVED'). Defaults to 'OPEN'."),
});
