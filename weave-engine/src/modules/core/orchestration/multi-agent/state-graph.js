...
class StateGraph {
  ...
  compile() {
    ...
    if (!this.nodes.has(this.entryPoint)) {
      throw new Error(`StateGraph compilation failed: Entry point '${this.entryPoint}' is not a registered node.`);
    }
    return this;
  }
  ...
  async run(initialState, options = {}) {
    ...
    while (currentNode !== END && iterations < maxIterations) {
      ...
      if (!this.nodes.has(currentNode)) {
        throw new Error(`StateGraph execution failed: Node '${currentNode}' not found.`);
      }

      logger.debug("StateGraph executing node", { node: currentNode, iterations });
      ...
      try {
        ...
        logger.error(`StateGraph execution error at node '${currentNode}'`, { error: error.message });
      ...
      logger.debug("StateGraph transition", { from: currentNode, to: nextNode });
      currentNode = nextNode;
    }

    if (iterations >= maxIterations) {
      logger.warn("StateGraph execution hit max iterations limit", { maxIterations });
      currentState.errors = currentState.errors || [];
      currentState.errors.push(`Execution stopped after ${maxIterations} iterations (max limit).`);
    }

    logger.info("StateGraph run completed", { iterations, endNode: currentNode });
    return currentState;
  }
}
