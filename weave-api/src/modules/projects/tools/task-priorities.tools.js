...
const manageTaskPrioritiesSchema = z.discriminatedUnion("action", [
  z.object({
    ...
    level: z
      .number()
      .optional()
      .describe("Numerical priority level, e.g. 1, 2, 3"),
...
const createTaskPriorityTools = (user) => ({
  manage_task_priorities: {
    description:
      "Manage task priorities for a project (create, update, delete, list).",
    handler: async (args) => {
      try {
        ...
        if (action === "update") {
          if (!priorityId)
            throw new Error("priorityId is required for update action");
          ...
          if (!updated)
            throw new Error(`Task priority ${priorityId} not found.`);
          return {
        ...
        if (action === "delete") {
          if (!priorityId)
            throw new Error("priorityId is required for delete action");
