...
const manageNotificationsSchema = z.discriminatedUnion("action", [
  z.object({
    ...
    target_user_id: z
      ...
      .uuid()
      .optional()
      .describe("ID of the target user"),
  ...
  z.object({
    ...
    isRead: z
      .boolean()
      .optional()
      .describe("Whether to mark as read or unread"),
  ...
  manage_notifications: {
    ...
    handler: async (args) => {
      try {
        ...
        if (action === "list") {
          const result =
            await NotificationsReadRepository.listUserNotifications({
              userId: user.id,
              ...listArgs,
            });
          return {
        ...
        if (action === "mark_read") {
          if (!notificationId)
            throw new Error("notificationId is required for mark_read action.");
          const result =
            await NotificationsUpdateRepository.markNotificationRead({
