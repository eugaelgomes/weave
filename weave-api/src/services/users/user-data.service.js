...
const { normalizeEmail, normalizeUsername, normalizePhoneNumber } = require("@/modules/users/utils/unique-conflicts");
...
class UserDataService {
  async updateProfile(userId, currentUser, reqBody, reqFile, reqObj) {
    ...
    const {
      name, username, email, emailValidationToken, currentPassword, newPassword,
      theme_mode, birth_date, phone_number, private_profile, usage_preference, user_preference
    } = reqBody;
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (currentPassword && newPassword) {
        ...
        if (!match) {
          updateProfileLogs.createLog(userId, "security_change", reqObj, "failure", { reason: "wrong_current_password" });
          throw new Error("INCORRECT_PASSWORD");
        }
        const hashedPassword = await bcrypt.hash(newPassword, saltRounds);
        await UserDataRepository.updateUserPassword(userId, hashedPassword);
        auditChanges.password_changed = true;
        updateProfileLogs.createLog(userId, "security_change", reqObj, "success", { action: "password_update" });
      }

      // 2. Process Email Token Validation
      if (emailValidationToken) {
        const tokenRecord = await UserTokensRepository.findEmailChangeToken(userId, emailValidationToken);
        ...
        if (dataToUpdate?.new_email) {
          await UserDataRepository.updateUserProfile(userId, { email: dataToUpdate.new_email });
      ...
      if (email !== undefined && normalizeEmail(email) !== normalizeEmail(currentUser.email) && !emailValidationToken) {
        const emailAvailability = await SearchUsersRepository.checkUniqueAvailability({ email }, { excludeUserId: userId });
        ...
        await UserTokensRepository.createEmailChangeToken(userId, token, email, new Date().toISOString().slice(0, 19).replace("T", " "));
      ...
      if (resolvedPreference !== undefined) {
        const parsed = typeof resolvedPreference === "string" ? JSON.parse(resolvedPreference) : resolvedPreference;
        updates.user_preference = normalizeAppPreferences(parsed);
      }

      if (username !== undefined && normalizeUsername(username) !== normalizeUsername(currentUser.username)) {
        const usernameAvailability = await SearchUsersRepository.checkUniqueAvailability({ username }, { excludeUserId: userId });
        if (!usernameAvailability.username.available) throw new Error("CONFLICT_USERNAME");
        updates.username = username;
      }

      if (phone_number !== undefined && normalizePhoneNumber(phone_number) !== normalizePhoneNumber(currentUser.phone_number)) {
        const phoneAvailability = await SearchUsersRepository.checkUniqueAvailability({ phone_number }, { excludeUserId: userId });
    ...
    if (reqFile && reqFile.path) {
      ...
      const uploadResult = await spacesService.uploadProfileImage(fileStream, reqFile.mimetype, userId);
      
      fs.unlink(reqFile.path, (err) => { if (err) console.error("Failed to delete temp file:", err); });
