...
class AiReportSchedulerProcessor {
  ...
  /**
   * Only triggers the API. The API will gather data and talk to the engine.
   */
  async processConfig(config) {
    if (!config.sprint_id) return;

    const reportType = this.determineReportType(config);
    if (!reportType) {
      const nextAt = this.calculateNextReportAt(config);
      await this.updateSchedulerState(config.id, { next_report_at: nextAt });
      return;
    }
    ...
    const trigger = {
      ...
      reportType,
      reasoningType: reportType,
      title: `Sprint ${config.sprint_number || "?"} - ${this._reportTypeLabel(reportType)} (${new Date().toISOString().split("T")[0]})`,
      config, // Send full config for API to have context info
      triggeredAt: new Date().toISOString(),
    };

    await redis.rpush(REASONING_TRIGGER_QUEUE, JSON.stringify(trigger));
    logger.info("AI report scheduler: triggered API for reasoning", {
      projectId: config.project_id,
      reportType,
    });
