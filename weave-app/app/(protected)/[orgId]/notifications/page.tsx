...
import { Loader2, Bell, CheckCheck, RefreshCw, AlertCircle } from "lucide-react";
import { NotificationList } from "@/app/(protected)/[orgId]/notifications/_components/notification-list";
          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="flex animate-pulse items-start gap-4 p-4">
              ...
              <div className="flex-1 space-y-2">
                ...
                <div className="h-2 w-1/2 rounded bg-neutral-100 dark:bg-[#1d1d1b]"></div>
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="flex h-64 flex-col items-center justify-center rounded-md border border-red-200 bg-red-50/50 p-6 text-center shadow-sm transition-colors dark:border-red-900/30 dark:bg-red-900/10">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-md border border-red-200 bg-white px-4 py-1.5 text-[11px] font-bold text-red-600 shadow-sm transition-all hover:bg-red-50 active:scale-95 dark:border-red-900/50 dark:bg-[#1d1d1b] dark:text-red-400 dark:hover:bg-red-900/20"
        >
          <RefreshCw size={12} />
          Tentar novamente
        </button>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="dark:shadow-surface-dark-sm dark:border-surface-dark-border overflow-hidden rounded-md border border-neutral-200 bg-white shadow-sm dark:bg-[#1d1d1b]">
        ...
        <div className="relative min-h-[300px] bg-white dark:bg-[#1d1d1b]">
          {loading && !initialLoading && notifications.length > 0 && (
            <div className="absolute inset-0 z-10 flex items-start justify-center bg-white/50 pt-10 backdrop-blur-[1px] dark:bg-[#1d1d1b]/50">
