...
function flattenBlockTextsForDiscard(
...
const NoteDetail = () => {
  ...
  useEffect(() => {
    ...
    if (initialLoading || !note || note.id !== id || !note.associated_project?.id || !note.access?.canDelete) {
      return;
    }
  ...
  useEffect(() => {
    const sessionNoteId = id;
    return () => {
      if (userEditedEmptyProjectTaskRef.current) return;
      if (hasPendingBlockWritesRef.current) return;
      if (discardArmedForNoteIdRef.current !== sessionNoteId) return;
      void deleteNote(sessionNoteId).catch(() => {});
    };
  }, [id, deleteNote]);
