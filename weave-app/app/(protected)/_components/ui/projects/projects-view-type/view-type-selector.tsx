...
export default function ViewTypeSelector({
  ...
  if (variant === "select") {
    return (
      <select
        ...
        disabled={disabled}
        className={`dark:border-surface-dark-border-strong w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm font-medium text-neutral-900 transition-colors focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 sm:px-4 sm:py-2.5 dark:bg-neutral-800 dark:text-neutral-100 ${className}`}
      >
            ...
            <button
              ...
              className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-all disabled:cursor-not-allowed disabled:opacity-50 ${
                isSelected
                  ...
                  : "dark:border-surface-dark-border-strong border-neutral-300 bg-white text-neutral-700 hover:border-neutral-400 hover:bg-neutral-50 dark:bg-neutral-800 dark:text-neutral-300 dark:hover:border-neutral-600 dark:hover:bg-neutral-700"
              }`}
            >
  ...
  if (variant === "tabs") {
    return (
      <div
        className={`dark:border-surface-dark-border-strong inline-flex rounded-lg border border-neutral-200 bg-neutral-100 p-1 dark:bg-neutral-800 ${className}`}
      >
        {VIEW_TYPE_OPTIONS.map((option) => {
          const isSelected = value === option.value;
          return (
