...
content = content.replace('import SearchModal from "@/app/(protected)/_components/ui/navbar/search-modal";\n', '');
// Remove createPortal
...
if (customHooksStart !== -1 && principalStart !== -1) {
  ...
  content = content.substring(0, customHooksStart) + notifStr + "\n" + content.substring(principalStart);
}
...
content = content.replace(/useClickOutside\(\[desktopMenuRef, mobileMenuRef\], \(\) => setIsMenuOpen\(false\)\);\n/g, '');
const effMenuRegex = /useEffect\(\(\) => \{\n\s*document\.body\.style\.overflow = isMenuOpen \? "hidden" : "";\n\s*return \(\) => \{\n\s*document\.body\.style\.overflow = "";\n\s*\};\n\s*\}, \[isMenuOpen\]\);\n/;
content = content.replace(effMenuRegex, '');

// 5. Desktop search button
const desktopSearchRegex = /<div className="hidden w-full max-w-md items-center gap-2 md:flex">\n\s*<button\n\s*type="button"\n\s*onClick=\{\(\) => setIsSearchOpen\(true\)\}[\s\S]*?<\/button>/m;
content = content.replace(desktopSearchRegex, '<div className="hidden w-full max-w-md items-center justify-end gap-2 md:flex">'); // keeping the div for NotificationsLink

// 6. Mobile search button
const mobileSearchRegex = /<button\n\s*type="button"\n\s*onClick=\{\(\) => setIsSearchOpen\(true\)\}\n\s*className=\{cn\(navIconMobileShellClass, "md:hidden"\)\}\n\s*aria-label=\{t\.navbar\.openSearch\}\n\s*title=\{t\.navbar\.openSearch\}\n\s*>\n\s*<Search className="h-4 w-4" strokeWidth=\{1\.75\} \/>\n\s*<\/button>\n/m;
content = content.replace(mobileSearchRegex, '');

// 7. User Menu
const userMenuDivRegex = /<div className="relative self-center" ref=\{desktopMenuRef\}>[\s\S]*?<\/div>\n\s*<\/div>/m; // wait, the closing div might match too far
...
const searchModalAndMobileMenuRegex = /<SearchModal isOpen=\{isSearchOpen\} onClose=\{\(\) => setIsSearchOpen\(false\)\} \/>[\s\S]*?document\.body\n\s*\)\}/m;
