...
todos:
  - id: inspect-membership-shape
    content: Read `organizations.repository.getActiveOrganizationWithMembership` and confirm whether `membership.id` is org id or membership row id; identify correct field to pass to org-scope project queries.
    status: completed
  - id: inspect-org-scope-queries
    content: Read and verify `projects-read.repository.js` org-scope methods filter by `projects.organization_id` and accept the organization id correctly.
    status: completed
  - id: pinpoint-branch-taken
    ...
    status: completed
  - id: implement-fix
    content: Patch controllers/core access validation to use the correct organization id field; align org-scope repo filters if needed; keep backward compatibility if both old/new shapes exist.
    status: completed
...
- You confirmed:
  - DB baseline is `weave-api/db_structure_docs/new_structure_db.sql` (uppercase enums, `organization_id`, etc.)
...
- **Membership/org-scope mismatch**: `ProjectsCoreController._validateProjectAccess` has an org-wide path that calls `projectsRepository.getProjectByIdWithOrgScope(projectId, membership.id)`.
  - If `membership.id` is not actually the organization id (common shape is `organization_id`), org-scope lookups will always fail.
...
- **Repository org-scope query doesn’t match new schema**: `getProjectByIdWithOrgScope` may filter on the wrong column (e.g., `org_id` vs `organization_id`) even though other queries match new schema.
...
- Inspect organization membership shape and role policy:
  - [`weave-api/src/modules/organizations/repositories/organizations.repository.js`](weave-api/src/modules/organizations/repositories/organizations.repository.js) `getActiveOrganizationWithMembership` return shape.
  - [`weave-api/src/modules/organizations/organization-role-policy.js`](weave-api/src/modules/organizations/organization-role-policy.js) for `ACCESS_ALL_ORG_PROJECTS` logic.
...
- **Normalize membership organization id**:
  - Update `ProjectsCoreController._validateProjectAccess` and `ProjectsReadController.getCollaborators` to pass `membership.organization_id` (or equivalent) into org-scope repository methods.
  - Keep compatibility by supporting both shapes: `membership.organization_id ?? membership.org_id ?? membership.id` only if `membership.id` is confirmed to be org id.
- **Harden `_canAccessAllOrganizationProjects` usage**:
  - Ensure it only returns true for roles that actually have `ORG_PERMISSIONS.ACCESS_ALL_ORG_PROJECTS`.
- **Align org-scope repository filters to new schema**:
  - Ensure `getProjectByIdWithOrgScope` filters on `projects.organization_id` (not legacy `org_id`).
