...
todos:
  - id: doc-contract
    content: Reforçar em organizations-routes.md + README que Slack usa org ativa (paths sem :orgId); slack-integration.md como canónico.
    status: completed
  - id: doc-migrate-runbook
    content: Documentar comando psql (ou equivalente) para 2026-05-09_create_organization_slack_integrations.sql no README ou slack-integration.md.
    status: completed
  - id: env-slack
...
**Decisão de produto já refletida no código:** os endpoints de integração Slack seguem o mesmo padrão que o resto de [`weave-api/src/modules/organizations/organizations.routes.js`](weave-api/src/modules/organizations/organizations.routes.js): após `verifyToken`, a organização vem do **contexto ativo** do utilizador (`getActiveOrganizationWithMembership`), não de um `:orgId` na URL.

| Caminho real (v1) | Autenticação | Permissão |
|-------------------|--------------|-----------|
| `GET /api/v1/organizations/integrations/slack` | Bearer | `MANAGE_GLOBAL_INTEGRATIONS` (checado no controller) |
| `PUT /api/v1/organizations/integrations/slack/default-channel` | idem | idem |
| `DELETE /api/v1/organizations/integrations/slack` | idem | idem |
...
- Em [`weave-api/documents/routes/organizations-routes.md`](weave-api/documents/routes/organizations-routes.md), garantir uma linha explícita: *“Slack: escopo = organização ativa do JWT/membership; não usar `organizationId` no path.”*
...
flowchart LR
  ...
  subgraph api [weave-api]
    OrgsRoutes["/organizations/integrations/slack"]
...
O ficheiro já existe: [`weave-api/db_structure_docs/migrations/2026-05-09_create_organization_slack_integrations.sql`](weave-api/db_structure_docs/migrations/2026-05-09_create_organization_slack_integrations.sql).
...
2. Executar o ficheiro **uma vez** por ambiente (dev/staging/prod), por exemplo:
   - `psql "$DATABASE_URL" -f weave-api/db_structure_docs/migrations/2026-05-09_create_organization_slack_integrations.sql`
