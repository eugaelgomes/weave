              ...
              <div className="max-h-72 space-y-2 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-neutral-200 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-700">
                {availableNotes.map((note) => (
                  <button
                    ...
                    className="group relative flex w-full flex-col gap-1.5 rounded-lg border border-neutral-200 bg-white p-3 text-left transition-all hover:border-yellow-500/50 hover:bg-yellow-50/30 disabled:opacity-60 dark:border-neutral-800 dark:bg-neutral-950 dark:hover:border-yellow-500/30 dark:hover:bg-yellow-500/5"
                  >
                    <div className="flex w-full items-start justify-between gap-2">
                      <p className="text-xs font-semibold text-neutral-800 transition-colors group-hover:text-yellow-600 dark:text-neutral-200 dark:group-hover:text-yellow-500">
                        {note.title}
                      </p>
                      {isAdding === note.id ? (
                        <FaSpinner className="h-3 w-3 animate-spin text-yellow-500" />
                      ) : (
                        <Plus className="h-3.5 w-3.5 text-neutral-400 opacity-0 transition-all group-hover:text-yellow-500 group-hover:opacity-100" />
                      )}
                    </div>
