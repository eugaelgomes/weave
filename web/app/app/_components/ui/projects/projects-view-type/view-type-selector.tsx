...
export default function ViewTypeSelector({
  ...
  if (variant === "select") {
    return (
      <select
        ...
        disabled={disabled}
        className={`w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm font-medium text-neutral-900 transition-colors focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 sm:px-4 sm:py-2.5 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100 ${className}`}
      >
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-all disabled:cursor-not-allowed disabled:opacity-50 ${
                isSelected
                  ? "border-yellow-500 bg-brand-primary-500/10 text-yellow-600 dark:border-yellow-500 dark:bg-brand-primary-500/20 dark:text-yellow-400"
  ...
  if (variant === "tabs") {
    return (
      <div
        className={`inline-flex rounded-lg border border-neutral-200 bg-neutral-100 p-1 dark:border-neutral-700 dark:bg-neutral-800 ${className}`}
      >
        {VIEW_TYPE_OPTIONS.map((option) => {
          const isSelected = value === option.value;
          return (
