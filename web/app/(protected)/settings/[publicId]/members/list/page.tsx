...
import { WORKSPACE_ROLES } from "@/app/_services/workspace";
...
const MEMBERSHIP_STATUSES: WorkspaceMember["membership"]["status"][] = ["active", "pending"];
...
type RoleManageModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (memberId: string, newRole: WorkspaceRole) => void;
  loading: boolean;
...
const RoleManageModal = ({
  ...
  const roleOptions = React.useMemo(
    () => WORKSPACE_ROLES.filter((r) => assignSuperAdmin || r !== "SUPER_ADMIN"),
    [assignSuperAdmin]
  );
  ...
  React.useEffect(() => {
    if (currentMember) {
      ...
      setRole(r && roleOptions.includes(r) ? r : (roleOptions[0] ?? "MEMBER"));
    }
  ...
  const workspaceRoleLabel = (r: WorkspaceRole): string => {
      ...
      <div className="space-y-4">
        <div className="dark:border-surface-dark-border flex items-center gap-3 rounded-md border border-neutral-200 p-2">
          ...
          <div>
            ...
            <MemberWorkspaceRoleBadge role={currentMember.membership?.role || "MEMBER"} />
          </div>
        </div>
        <div>
...
export default function MembersPage() {
  ...
  const roleFilterOptions = React.useMemo((): FilterSelectOption[] => {
    ...
    return WORKSPACE_ROLES.map((r) => ({ value: r, label: labels[r] }));
  }, [t]);

  const statusFilterOptions = React.useMemo((): FilterSelectOption[] => {
    const labels: Record<WorkspaceMember["membership"]["status"], string> = {
