...
function WeaveAiSidebar({ onLinkClick }: { onLinkClick?: () => void }) {
  ...
  const handleDeleteSession = async (sessionId: string) => {
    ...
    if (currentSession?.id === sessionId) {
      router.push(routes.weaveAi.chat());
      onLinkClick?.();
    }
  };
    ...
    <div className="flex w-full flex-col px-2 py-3">
      ...
      <div className="mb-6">
        ...
        <button
          type="button"
          onClick={() => {
            router.push(routes.weaveAi.newAgent());
            onLinkClick?.();
          }}
            ...
            return (
              <li key={agent.id}>
                <button
                  type="button"
                  onClick={() => {
                    router.push(routes.weaveAi.agentDetails(agent.id));
                    onLinkClick?.();
                  }}
                  className={cn(
            ...
            <>
              {chatHistory.map((chat) => (
                ...
                >
                  <button
                    onClick={() => {
                      router.push(routes.weaveAi.chatSession(chat.id));
                      onLinkClick?.();
                    }}
                    className={cn(
