...
import { useCalendarPageView } from "@/app/(protected)/[orgId]/calendar/_contexts/calendar-page-view-context";
...
function MicroCalendarPeopleFreeBusy({
  ...
  return (
    <div className="relative mt-2">
      <div className="dark:border-surface-dark-border-strong flex w-full items-center gap-1.5 rounded-md border border-neutral-200 bg-white px-2 py-1.5 dark:bg-[#1d1d1b]/60">
      ...
      {query.trim().length >= 3 && results.length > 0 ? (
        <ul className="dark:shadow-surface-dark-md dark:border-surface-dark-border-strong absolute right-0 left-0 z-30 mt-0.5 max-h-32 overflow-y-auto rounded-md border border-neutral-200 bg-white py-0.5 shadow-md dark:bg-[#1d1d1b]">
          {results.map((u) => (
            <li key={u.id}>
              <button
                type="button"
      ...
      {pickedEmail && pickedName ? (
        <div className="dark:border-surface-dark-border-strong mt-2 space-y-1.5 rounded-md border border-neutral-200 bg-white/90 p-2 dark:bg-[#1d1d1b]/70">
