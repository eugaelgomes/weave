...
import { IntegrationsSection } from "@/app/(protected)/organization/_components/integrations-section";
import { DomainsSection } from "@/app/(protected)/organization/integrations/_components/domains-section";

export default function OrganizationIntegrationsPage() {
  const { t } = useLanguage();
  const { user } = useAuth();
  const { workspace, hasOrganization, isOwner } = useOrganization();
