...
export async function executeListNotes(args) {
    try {
        const params = {};
        if (args?.projectId)
        ...
        const notes = await weaveClient.get("/notes", params);
        return {
            content: [
                {
                    type: "text",
...
export async function executeSearchNotes(args) {
    try {
...
export async function executeGetNote(args) {
    try {
        const note = await weaveClient.get(`/notes/${args.id}`);
        return {
            content: [
                {
                    type: "text",
...
export async function executeCreateNote(args) {
    try {
        const note = await weaveClient.post("/notes", args);
        return {
            content: [
                {
                    type: "text",
