...
jobs:
  validate-worker:
    ...
    defaults:
      run:
        working-directory: weave-worker
    ...
    steps:
      ...
      - name: Setup Node.js
        ...
        with:
          node-version: 20
          cache: npm
          cache-dependency-path: weave-worker/package-lock.json
      ...
      - name: Deploy via SSH
        ...
        with:
          ...
          script: |
            ...
            if [ ! -f .env ] && [ ! -f weave-worker/.env ]; then
              echo "Need .env or weave-worker/.env on VM (DOPPLER_TOKEN / runtime secrets)." >&2
              exit 1
            fi
