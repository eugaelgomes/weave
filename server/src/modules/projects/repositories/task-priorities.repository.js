...
class TaskPrioritiesRepository {
   ...
   * @returns {Promise<any>}
   */
  async createPriority({ projectId = null, orgId = null, name, color, level, createdBy }) {
    const query = `
      INSERT INTO task_priorities (project_id, organization_id, name, color_hex, sort_order, user_id)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *;
    `;
    const result = await executeQuery(query, [projectId, orgId, name, color, level, createdBy]);
    return result[0];
  }
  ...
  async updatePriority(priorityId, { projectId = null, orgId = null, updates }) {
    ...
    const query = `
      ...
      RETURNING *;
    `;
    const result = await executeQuery(query, [name, color, level, priorityId, projectId, orgId]);
    return result[0];
  }
  ...
  async deletePriority(priorityId, { projectId = null, orgId = null, deletedBy }) {
    const query = `
      ...
      RETURNING *;
    `;
    const result = await executeQuery(query, [deletedBy, priorityId, projectId, orgId]);
    return result[0];
  }
}
