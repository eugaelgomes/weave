...
const manageCalendarSchema = z.discriminatedUnion("action", [
  z
...
const createCalendarTools = (user) => ({
  manage_calendar: {
   ...
   - What it does: Returns the user's Google Calendar settings (e.g. timezone).`,
    handler: async (args) => {
      try {
        ...
        if (action === "create") {
          const event = await calendarService.createEvent(args, userId);
          return {
        ...
        if (action === "check_free_busy") {
          const freebusy = await calendarService.checkFreeBusy(userId, {
            items,
            timeMax: time_max,
            timeMin: time_min,
          });
